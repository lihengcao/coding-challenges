...
edges = {
    k: {vk: distance(k, vk) for vk in nonzero_valves if vk != k} for k in nonzero_valves
}
...
def dfs1(
    ...
    time: int = 30 + 1,
    pressure: int = 0,
    visited: frozenset[str] = frozenset(("AA",)),
) -> int:
    if time <= 0:
        return pressure
    ...
    return max(
        dfs1(new_valve, time - edges[valve][new_valve] - 1, pressure, new_visited)
        for new_valve in not_visited
    )
